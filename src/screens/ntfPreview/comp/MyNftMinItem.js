...
const MyNftMinItem = ({item, index, symbol, isImage}) => {
  ...
  useEffect(() => {
    getNftItemInfo(item.collection_address, item.nft_token_address).then(
      res => {
  ...
  // console.log('ddddd', item.nft_token_address);
  return (
    <View style={[styles.container, FG, {width: itemWidth}, itemMargin]}>
      ...
      <Text style={[text, styles.text2]}>{`#${item?.nft_token_address}`}</Text>
    </View>
  );
};
